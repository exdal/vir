...
/// The slots the compiled draws read.
struct Slots {
    has_corner_triangle: ValueId,
    push: ValueId,
    /// The tail of the block, which the corner draw pushes on its own.
    tint: ValueId,
}
...
impl Example for Triangle {
    ...
    /// Both draws go straight into the image the frame acquires, and neither the clear nor the
    /// layouts around it are written here: the module the UI is compiled into is the same one,
    /// so the graph has the whole frame to order.
    fn record(&mut self, recording: &mut Recording) -> Result<ValueId, vk::Result> {
        ...
        let tint = module.declare_bytes_var("corner tint", size_of::<f32>() as u32);
        ...
        // whether the corner triangle is drawn at all is a branch on a variable rather than a
        // Rust `if`, so turning it off does not mean compiling a second graph
    ...
    fn update(&mut self, frame: &mut Frame) -> Result<(), vk::Result> {
        ...
        frame.program.set(slots.has_corner_triangle, self.ui.corner);
